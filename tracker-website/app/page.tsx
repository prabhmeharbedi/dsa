...
export default function Home() {
  ...
  // Cloud-synced completion state (global across all devices via /api/state)
  ...
  // Load from server on mount (authoritative) with local fallback
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        // Fallback to localStorage if available (dev/offline)
        try {
          const raw = localStorage.getItem("tracker-completion");
          if (raw) setCompletion(JSON.parse(raw));
          const ts = localStorage.getItem("tracker-last-synced");
  ...
  // Debounced save to cloud + local backup
  useEffect(() => {
    if (loading) return;

    // local backup
    try {
      localStorage.setItem("tracker-completion", JSON.stringify(completion));
    } catch {}
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        setLastSynced(ts);
        try {
          localStorage.setItem("tracker-last-synced", ts);
        } catch {}
      } catch (e: any) {
  ...
  // Totals and progress calculations derived from current completion state
    ...
    weeks.forEach((week) => {
      ...
      week.days.forEach((day) => {
        ...
        day.system_design_tasks.forEach((_, idx) => {
          totalSD += 1;
          wTotal += 1;
          if (completion[makeSDKey(week.number, day.number, idx)]) {
            doneSD += 1;
            wDone += 1;
          }
        });
